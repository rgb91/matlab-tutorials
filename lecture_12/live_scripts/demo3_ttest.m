...
%  p tells you IF the groups differ; d tells you HOW MUCH, independent of n.
n1 = numel(control);
n2 = numel(treatment);
pooled_std = sqrt(((n1-1)*std(control)^2 + (n2-1)*std(treatment)^2) / (n1+n2-2));
d = (mean(control) - mean(treatment)) / pooled_std;
fprintf('Cohen''s d = %.2f\n', d);


%% BONUS — One-way ANOVA
%  anova1 needs data in one column + a matching group-label vector/cell array.
load('data/group_data.mat');   % control, treatment (1x20 each)

data   = [control, treatment]';                 % stack into one column, 40x1
groups = [repmat({'control'},20,1); repmat({'treatment'},20,1)];  % labels

[p, tbl, stats] = anova1(data, groups, 'off');  % 'off' suppresses auto-popup plots
fprintf('ANOVA: F(%d,%d) = %.2f, p = %.4g\n', tbl{2,3}, tbl{3,3}, tbl{2,5}, p);